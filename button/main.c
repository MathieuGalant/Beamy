...
char* ReadXmlFile(char *filename,char *attribut[10]);
void getXml(xmlNode * node, int i,char *attribut[10]);
int is_leaf(xmlNode * node);
void modifyXml(char * filename);
void getFolderFiles(char *foldername,char *musicName[10]);
void playMusic(FMOD_SYSTEM *system, char musicName[100], FMOD_CREATESOUNDEXINFO exinfo, FMOD_SOUND *music, FMOD_CHANNEL *channel, float volume);
...
int main(int argc, char **argv)
{
    ...
    playMusic(system,musicPlaying,exinfo,music,channel,volume);
...
void readXmlFile(char *filename,  char * attribut[10])
{
    xmlDoc *doc = NULL;
...
{
    FMOD_RESULT resultat;
    resultat=FMOD_System_CreateStream(system, musicName, FMOD_LOOP_NORMAL | FMOD_2D, &exinfo, &music);
